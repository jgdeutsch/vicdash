...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Get existing cached data to preserve sends/opens data
    const cached = await getCachedStats().catch(() => ({ campaigns: {} }));
    const existingCampaigns = cached.campaigns || {};
    
    // Collect only leads
    const newData = await collectAllCampaigns(send, ids, { includeSendsOpens: false, includeLeads: true });
    
    // Merge: use new leads data, preserve existing sends/opens data

  ...
  do {
    ...
    while (true) {
      ...
      if (resp.status === 429 || (resp.status >= 500 && resp.status < 600)) {
        ...
        tries++;
        log?.(`rate-limited/server error, retrying in ${retryAfterMinutes} minute${retryAfterMinutes !== 1 ? 's' : ''} (attempt ${tries})`);
...
export async function discoverCampaigns(searchTerm, apiKey, log) {
  log?.(`Discovering campaigns with search term: "${searchTerm}"`);
  ...
  log?.(`Found ${campaignIds.length} campaigns matching "${searchTerm}"`);
  return campaignIds;
}
...
export async function collectAllCampaigns(log, overrideIds, options = {}) {
  ...
  if (Array.isArray(overrideIds) && overrideIds.length) {
    ...
    log?.(`Discovering campaigns with "[VB]" in title...`);
    ids = await discoverCampaigns('[VB]', apiKey, log);
    if (ids.length === 0) {
      log?.('No campaigns found with "[VB]" in title, using fallback from config');

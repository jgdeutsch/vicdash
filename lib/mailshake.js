...
export async function fetchPaginated(endpoint, params = {}, apiKey, log) {
  ...
  return {
    results,
    paginated: hadMultiplePages || pageCount > 1,
    pageCount,
...
export async function discoverCampaigns(searchTerm, apiKey, log) {
  ...
  const data = await fetchPaginated('campaigns/list', { search: searchTerm }, apiKey, log);
  const campaignIds = data.results.map(c => Number(c.id)).filter(Boolean);
...
export async function collectCampaignStats(campaignId, apiKey, log, options = {}) {
  ...
  let opensPaginated = false, opensReachedEnd = false;
  let repliesPaginated = false, repliesReachedEnd = false;

  if (includeSendsOpens) {
    formatLogMessage(`Campaign ${campaignId}: fetching sends`, log);
    const sentData = await fetchPaginated('activity/sent', { campaignID: String(campaignId) }, apiKey, log);
    sends = sentData.results.length;
    formatLogMessage(`Campaign ${campaignId}: found ${sends} send(s)`, log);

    formatLogMessage(`Campaign ${campaignId}: fetching opens`, log);
    const opensData = await fetchPaginated('activity/opens', { campaignID: String(campaignId) }, apiKey, log);
    const opens = opensData.results;
    opensPaginated = opensData.paginated;
    opensReachedEnd = opensData.reachedEnd;
    ...
    const repliesData = await fetchPaginated('activity/replies', { campaignID: String(campaignId), replyType: 'reply' }, apiKey, log);
    replies = repliesData.results.length;
    repliesPaginated = repliesData.paginated;
    repliesReachedEnd = repliesData.reachedEnd;
  ...
  if (includeLeads) {
    ...
    const wonData = await fetchPaginated('leads/list', { campaignID: String(campaignId), status: 'closed' }, apiKey, log);
    won = wonData.results.length;
    const lostData = await fetchPaginated('leads/list', { campaignID: String(campaignId), status: 'lost' }, apiKey, log);
    lost = lostData.results.length;
    const openLeadsData = await fetchPaginated('leads/list', { campaignID: String(campaignId), status: 'open' }, apiKey, log);
    openLeads = openLeadsData.results.length;
  }
  ...
  // Stats are considered complete if opens and replies reached the end (fully paginated)
  // If there were multiple pages, we verify both paginated and reached end
  // If single page, reaching end is sufficient (no nextToken means we got all data)
  const statsComplete = opensReachedEnd && repliesReachedEnd;
...
export async function collectAllCampaigns(log, overrideIds, options = {}) {
  ...
  // Get existing campaign data for sorting (if provided)
  const existingCampaigns = options.existingCampaigns || {};
  ...
  let campaignsToProcess = campaignsToRefresh.filter(id => !completedInSession.has(String(id)));
  ...
  // Sort campaigns if we have existing data and this is a sends/opens refresh
  if (options.includeSendsOpens && !options.includeLeads && Object.keys(existingCampaigns).length > 0) {
    campaignsToProcess.sort((a, b) => {
      const sendsA = existingCampaigns[String(a)]?.stats?.sends || 0;
      const sendsB = existingCampaigns[String(b)]?.stats?.sends || 0;
      
      // Campaigns with 0 sends come first
      if (sendsA === 0 && sendsB === 0) {
        return 0; // Keep original order for 0-send campaigns
      }
      if (sendsA === 0) return -1;
      if (sendsB === 0) return 1;
      
      // Then sort by sends ascending
      return sendsA - sendsB;
    });
    
    const zeroSendsCount = campaignsToProcess.filter(id => (existingCampaigns[String(id)]?.stats?.sends || 0) === 0).length;
    formatLogMessage(`Processing ${campaignsToProcess.length} campaign(s) that need refreshing: ${zeroSendsCount} with 0 sends first, then ascending by sends count${resumedCount > 0 ? ` (${resumedCount} already done)` : ''}...`, log);
  } else {
  ...
  for (let i = 0; i < campaignsToProcess.length; i++) {
    ...
    try {
      ...
      campaigns[c.id] = { title: c.title, sender: c.sender, stats: c.stats, statsComplete: c.statsComplete };

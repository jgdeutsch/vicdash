...
export async function collectAllCampaigns(log, overrideIds, options = {}) {
  ...
  for (const id of ids) {
    ...
    if (wasRecent) {
      formatLogMessage(`Skipping campaign ${id} - refreshed within last 12 hours`, log);
      skippedCount++;
      // Still include it in the result if we have cached data
      // (we'll merge with existing data in the refresh endpoints)
      continue;
    }
    
    formatLogMessage(`Processing campaign ${id}`, log);
    try {
  ...
  if (skippedCount > 0) {
    formatLogMessage(`Skipped ${skippedCount} campaign(s) that were refreshed in the last 12 hours`, log);
  }

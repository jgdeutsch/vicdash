...
// Klaviyo Event Checker
document.getElementById('klaviyo-check').addEventListener('click', async () => {
  const emailInput = document.getElementById('klaviyo-email');
  const checkBtn = document.getElementById('klaviyo-check');
  const resultsDiv = document.getElementById('klaviyo-results');
  const errorDiv = document.getElementById('klaviyo-error');
  ...
  // Get auth from browser (same as dashboard)
  let authHeader = localStorage.getItem('dashboardAuth') || '';
  if (!authHeader) {
    // Try to get from existing session or prompt
    const password = prompt('Enter dashboard password:');
    if (password) {
      authHeader = btoa(`user:${password}`);
      localStorage.setItem('dashboardAuth', authHeader);
    }
  }
  
  try {
    const res = await fetch('/api/check-klaviyo-events', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': authHeader ? `Basic ${authHeader}` : '',
      },
    ...
    if (!res.ok) {
      if (res.status === 401 && !authHeader) {
        ...
        if (password) {
          ...
          localStorage.setItem('dashboardAuth', encoded);
          // Retry with auth
          const retryRes = await fetch('/api/check-klaviyo-events', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
    ...
    checkBtn.textContent = 'Check Events';
  }
});
...
function displayResults(data) {
  ...
  const subscriptionEl = document.getElementById('check-subscription');
  subscriptionEl.textContent = events.subscriptionCreated ? '✅' : '❌';
  subscriptionEl.className = `check-result ${events.subscriptionCreated}`;

  const labtestEl = document.getElementById('check-labtest');
  labtestEl.textContent = events.labTestScheduled ? '✅' : '❌';
  labtestEl.className = `check-result ${events.labTestScheduled}`;

  const aiapEl = document.getElementById('check-aiap');
  aiapEl.textContent = events.viewedAiap ? '✅' : '❌';
  aiapEl.className = `check-result ${events.viewedAiap}`;
}

// Allow Enter key to submit
document.getElementById('klaviyo-email').addEventListener('keypress', (e) => {
  if (e.key === 'Enter') {
    document.getElementById('klaviyo-check').click();
  }
});
